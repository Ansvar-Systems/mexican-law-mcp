/**
 * search_eu_implementations — Search EU directives/regulations referenced by Mexican legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_mexican_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as mexican_statute_count
  ...
  if (input.has_mexican_implementation) {
    sql += ' HAVING mexican_statute_count > 0';
  }
