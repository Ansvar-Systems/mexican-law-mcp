/**
 * get_mexican_implementations — Find Mexican statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetMexicanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface MexicanImplementationResult {
  document_id: string;
...
export async function getMexicanImplementations(
  db: InstanceType<typeof Database>,
  input: GetMexicanImplementationsInput,
): Promise<ToolResponse<MexicanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as MexicanImplementationResult[];
