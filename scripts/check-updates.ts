#!/usr/bin/env tsx
/**
 * Mexican Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.diputados.gob.mx/LeyesBiblio';
const PORTAL_NAME = 'Camara de Diputados (Mexico)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/mexican-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Mexican Law MCP — Data Freshness Check');
