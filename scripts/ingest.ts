 ...
 * Downloads DOC files from diputados.gob.mx and extracts text via antiword,
 * then parses the structured content into provision-level JSON seed files.
 *
...
import { fetchLawFile, docToText } from './lib/fetcher.js';
  ...
  for (const law of laws) {
    ...
    try {
      ...
      if (skipFetch && fs.existsSync(sourceFile)) {
        ...
        // Download DOC and extract text via antiword
        process.stdout.write(`  [${processed + 1}/${laws.length}] Fetching ${law.shortName} (${law.code})...`);
        const result = await fetchLawFile(law.code, law.docUrl);
        ...
        console.log(` DOC ${(result.buffer.length / 1024).toFixed(0)} KB`);

        process.stdout.write(`    Extracting text (antiword)...`);
        const tempFile = path.join(SOURCE_DIR, `${law.id}.doc`);
...
async function main(): Promise<void> {
  ...
  console.log(`  Method:  DOC download + antiword text extraction`);
